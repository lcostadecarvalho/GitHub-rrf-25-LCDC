* RRF 2025 - Construction Data Template	
*-------------------------------------------------------------------------------	
* Data construction: HH 
*------------------------------------------------------------------------------- 
	// Load household-level data (HH)
	use "${inter}/TZA_CCT_HH.dta", clear
	...
	generate 	area_acre = ar_farm 				if ar_unit == 1 , after(ar_farm) // lcdc: adds the variable right after ar_farm (like right order)
	...
	* Consumption in usd
	...
	foreach win_var of local winvars {
		...
		winsor 	`win_var', p(0.05) high gen(`win_var'_w) // high means only applies for upper tail
...
*------------------------------------------------------------------------------- 	
	...
	use "${inter}/TZA_CCT_HH_mem.dta", clear
	collapse 	(sum) treat_cost ///
				(max) read sick ///
				(mean) m_cost = treat_cost days_sick, by(hhid)
				
	replace treat_cost = m_cost if mi(m_cost) // because the sum changes it to 0	
	...
	lab var sick 		        "Any member was sick in last 4 weeks"
	lab var days_sick 		    "Average sick days"
	lab var treat_cost_usd 		"Total treatment cost in USD"
	
	drop m_cost treat_cost 
...
*------------------------------------------------------------------------------- 	
	...
	merge 1:1 hhid using `mem', assert(3) nogen 
 	...
 	merge m:1 vid using "${raw}/treat_status.dta", assert(3) nogen 

	
			//Save data
	save "${final}/TZA_CCT_analysis.dta", replace		
	

*-------------------------------------------------------------------------------	
* Data construction: Secondary data
*------------------------------------------------------------------------------- 	
	use "${inter}/TZA_amenity_tidy.dta", clear
	...
	// Exercise 6: Save final dataset ----
		// Instructions:
			...
			//Save data
	save "${final}/TZA_amenity_analysis.dta", replace
